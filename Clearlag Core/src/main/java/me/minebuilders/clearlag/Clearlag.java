...
import me.minebuilders.clearlag.hooks.ClearlagPlaceholders;
...
public class Clearlag extends JavaPlugin {
    ...
    private void initMetrics() {
        int pluginId = 11111; // Replace with actual ID after registration
        try {
            new Metrics(this, pluginId);
            Util.log("bStats metrics initialized");
        } catch (Exception e) {
            Util.warning("Failed to initialize bStats: " + e.getMessage());
        }
    }

    private void initPlaceholderAPI() {
        if (Bukkit.getPluginManager().getPlugin("PlaceholderAPI") != null) {
            new ClearlagPlaceholders(this).register();
            Util.log("PlaceholderAPI hook registered");
        }
    }
